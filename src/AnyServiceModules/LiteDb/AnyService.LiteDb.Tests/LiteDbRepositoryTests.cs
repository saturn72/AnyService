...
namespace AnyService.LiteDb.Tests
{
    public class LiteDbRepositoryTests
    {
        ...
        public async Task Insert()
        {
            ...
            var lr = new LiteDbRepository<TesTEntity>(dbName);
            ...
            var dbRes = (await lr.Insert(data)) as TesTEntity;
        ...
        public async Task GetById()
        {
            ...
            var entities = new[]{
                ...
                new TesTEntity{Id = "3", Value = "2"},
            };

            using (var db = new LiteDatabase(dbName))
            {
                db.GetCollection<TesTEntity>().Insert(entities);
            }
            var lr = new LiteDbRepository<TesTEntity>(dbName);
        ...
        public async Task Update()
        {
            ...
            var lr = new LiteDbRepository<TesTEntity>(dbName);
            var toUpdate = new TesTEntity { Id = "1", Value = "new Value" };
            var e = await lr.Update(toUpdate);
            e.ShouldBe(toUpdate);

            using (var db = new LiteDatabase(dbName))
            {
                var e1 = db.GetCollection<TesTEntity>().FindById(toUpdate.Id);
        ...
        public async Task GetAll()
        {
            ...
            var q = $"{nameof(TesTEntity.Value)} == 2";
        ...
        public async Task Delete()
        {
            ...
            using (var db = new LiteDatabase(dbName))
            {
                db.GetCollection<TesTEntity>().Count().ShouldBe(entities.Length - 1);
            }
        }
    }
}
